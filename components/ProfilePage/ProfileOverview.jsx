...
export default function ProfileOverview({ onSubscriptionClick, onSettingsClick, subscriptionName, timeUntilReset, stats }) {
  ...
  return (
    <div className={styles.overview}>
      <div 
        className={`${styles.subscriptionCard} ${hasSubscription && !isBasicSubscription ? styles.subscriptionCardActive : ''}`} 
        onClick={onSubscriptionClick}
      >
        <div className={styles.subscriptionCardIcon}>
          {hasSubscription && !isBasicSubscription ? (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 6L9 17L4 12" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          ) : (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="#1775F1" opacity="0.2"/>
              <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" stroke="#1775F1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          )}
        </div>
        <div className={styles.subscriptionCardContent}>
          <p className={styles.subscriptionCardTitle}>
            {isBasicSubscription ? "Оформить подписку" : subscriptionName}
          </p>
          <p className={styles.subscriptionCardSubtitle}>
            {isBasicSubscription 
              ? `Обновление лимитов через ${resetTimeFormatted}`
              : `Активна • Обновление лимитов через ${resetTimeFormatted}`
            }
          </p>
        </div>
        <div className={styles.subscriptionCardArrow}>
      ...
      <div className={styles.settingsCard} onClick={onSettingsClick}>
