...
import { getTelegramUserId, getTelegramUserIdWithRetry, getTelegramUser, initTelegramWebApp, isRunningInTelegram } from '@/lib/telegram'
...
const LOGIST_ROLE = 'logistic'
const ROLE_TELEGRAM_OFFSET = 100000000000000 // Для логистов - офсет для telegram_id в БД

// Функция для кодирования telegram_id для логистов (добавляет офсет)
function encodeTelegramIdForStorage(telegramId, role = LOGIST_ROLE) {
  if (!telegramId) {
    console.warn('[encodeTelegramIdForStorage] telegramId не передан')
    return null
  }
  const numId = Number(telegramId)
  if (isNaN(numId)) {
    console.warn('[encodeTelegramIdForStorage] Не удалось преобразовать telegramId в число:', telegramId)
    return null
  }
  if (role === LOGIST_ROLE) {
    return numId + ROLE_TELEGRAM_OFFSET
  }
  return numId
}

const LOGIST_NOTIFICATION_TYPES = new Set([
  'new_response',
  'response_accepted',
  'order_confirmed',
  'order_cancelled',
  'performer_cancelled_order',
  'pending_rating_logist',
  'order_finalized',
  'new_message'
])
...
const getNotificationType = (notification) => {
  ...
  if (notification.payload) {
    try {
      ...
      console.warn('[AppContext] Ошибка парсинга payload уведомления:', error, notification?.id)
    }
  }
...
const isLogistNotification = (notification) => {
  const type = getNotificationType(notification)
  if (!type) {
    return true
  }
  return LOGIST_NOTIFICATION_TYPES.has(type)
}
...
export function AppProvider({ children }) {
  ...
  const getUserId = useCallback(() => {
    // const fallbackId = "78783871611111"
    
    try {
      // Проверяем, доступен ли Telegram WebApp
      if (!isRunningInTelegram()) {
        console.log('[AppContext] Приложение не запущено в Telegram')
        return null
      }
      ...
      // Если Telegram ID недоступен
      console.warn('[AppContext] ⚠️ Telegram ID недоступен или равен null/undefined')
      return null
  ...
  // Асинхронная попытка получить правильный Telegram ID с повторными попытками
  useEffect(() => {
    // const fallbackId = "787838716"
    
    // Если уже получили Telegram ID, не делаем повторные попытки
    if (userId && userId !== null) {
      console.log(`[AppContext] Уже получен Telegram ID: ${userId}, пропускаем повторные попытки`)
      return
    }
    
    // Делаем повторные попытки получить Telegram ID
    const attemptGetTelegramId = async () => {
      console.log(`[AppContext] Начинаем повторные попытки получения Telegram ID...`)
      const telegramUserId = await getTelegramUserIdWithRetry(null, 5, 200)
      
      if (telegramUserId && telegramUserId !== userId) {
        console.log(`[AppContext] ✅ Обновляем userId с ${userId} на ${telegramUserId}`)
        setUserId(telegramUserId)
      } else {
        console.log(`[AppContext] ⚠️ Не удалось получить Telegram ID`)
      }
    }
    
    // Запускаем попытки после небольшой задержки, чтобы дать Telegram API время загрузиться
    const timeoutId = setTimeout(attemptGetTelegramId, 100)
  ...
  const loadUserBalance = useCallback(async () => {
    ...
    // Используем закодированный ID для логистов (в БД хранится telegram_id + офсет)
    const encodedUserId = encodeTelegramIdForStorage(userId, LOGIST_ROLE)
    if (!encodedUserId) {
      console.error("[loadUserBalance] Не удалось закодировать userId:", userId)
      return { success: false, error: "Invalid user ID" }
    }
    console.log("[loadUserBalance] userId:", userId, "encodedUserId:", encodedUserId)
    ...
    if (resp?.success) {
      ...
      console.log("[loadUserBalance] Транзакции из ответа:", resp.transactions?.length || 0, resp.transactions)
      setOperations(resp.transactions || [])
      console.log("[loadUserBalance] Установлено операций:", resp.transactions?.length || 0)
      
      // Обновляем информацию о пользователе если она пришла
      if (resp.user) {
        console.log("[loadUserBalance] Данные пользователя:", resp.user)
        console.log("[loadUserBalance] resp.user.subscription:", resp.user.subscription)
        console.log("[loadUserBalance] resp.user.subscription_id:", resp.user.subscription_id)
        console.log("[loadUserBalance] resp.user.subscription_tier:", resp.user.subscription_tier)
        ...
        console.log("[loadUserBalance] Устанавливаем profile с subscription:", newProfile.subscription)
  ...
  const loadUserOrders = useCallback(async () => {
    ...
    try {
      // Используем закодированный ID для запроса к API (в БД для логистов хранится telegram_id + офсет)
      // Но в orders.created_by хранится оригинальный telegram_id, поэтому для фильтрации используем оригинальный
      const encodedUserId = encodeTelegramIdForStorage(userId, LOGIST_ROLE)
      if (!encodedUserId) {
        console.error("[loadUserOrders] Не удалось закодировать userId:", userId)
        return
      }
      console.log("[loadUserOrders] Загрузка заказов для логиста. userId:", userId, "encodedUserId:", encodedUserId)
      const resp = await callApi({
        action: "getLogistOrders",
        logist_id: encodedUserId
      })
  ...
  const initRealtimeSubscriptions = useCallback((client) => {
    ...
    const logistId = Number(userId)
    const encodedLogistId = encodeTelegramIdForStorage(logistId, LOGIST_ROLE)
    if (!encodedLogistId) {
      console.error("[initRealtimeSubscriptions] Не удалось закодировать userId:", userId)
      return
    }
    console.log("[initRealtimeSubscriptions] logistId (Number):", logistId, "тип:", typeof logistId)
    console.log("[initRealtimeSubscriptions] encodedLogistId для БД:", encodedLogistId)
    ...
    // В orders.created_by хранится оригинальный telegram_id (без офсета)
    const ordersChannel = client
      ...
      .subscribe((status, err) => {
        console.log('[Realtime] Статус подписки на orders:', status, err ? 'ошибка:' : '', err)
        if (status === 'SUBSCRIBED') {
          console.log('[Realtime] ✅ Подписка на orders активна, logistId:', logistId)
        } else if (status === 'CHANNEL_ERROR') {
          console.error('[Realtime] ❌ Ошибка подписки на orders:', err)
    ...
    const transactionsChannel = client
      ...
      .on('postgres_changes', {
        ...
        schema: 'public',
        table: 'transactions',
        filter: `user_id=eq.${encodedLogistId}`
      }, async (payload) => {
    ...
    const usersChannel = client
      ...
      .on('postgres_changes', {
        ...
        schema: 'public',
        table: 'users',
        filter: `telegram_id=eq.${encodedLogistId}`
      }, async (payload) => {
    ...
    console.log('[initRealtimeSubscriptions] encodedLogistId для фильтра:', encodedLogistId)
    console.log('[initRealtimeSubscriptions] Фильтр будет: user_id=eq.' + encodedLogistId)
    
    // Основная подписка С фильтром (используем encodedLogistId для БД)
    const notificationsChannel = client
      ...
      .on('postgres_changes', {
        ...
        console.log('[Realtime] encodedLogistId для фильтра:', encodedLogistId)
        console.log('[Realtime] payload.new?.user_id:', payload.new?.user_id)
        
        if (payload.new) {
          const newNotification = payload.new
          if (!isLogistNotification(newNotification)) {
            console.log('[Realtime] ⚠️ Уведомление проигнорировано для роли логиста. Тип:', getNotificationType(newNotification))
            return
          }
          ...
          // Проверяем, что уведомление для нашего пользователя (используем encodedLogistId, так как в БД хранится закодированный ID)
          const notificationUserId = Number(newNotification.user_id)
          if (notificationUserId !== encodedLogistId) {
            console.warn('[Realtime] ⚠️ Уведомление не для нашего пользователя:', {
              notificationUserId,
              encodedLogistId,
              match: notificationUserId === encodedLogistId
            })
            return
          }
      ...
      .subscribe((status, err) => {
        ...
        if (status === 'SUBSCRIBED') {
          console.log('[Realtime] ✅ Подписка на notifications активна для user_id:', logistId, '(encoded:', encodedLogistId + ')')
          console.log('[Realtime] ✅ Real-time подписка готова к получению уведомлений')
          console.log('[Realtime] ✅ Фильтр: user_id=eq.' + encodedLogistId)
    ...
    console.log("[initRealtimeSubscriptions] ✅ Подписка на notifications создана для user_id:", logistId, '(encoded:', encodedLogistId + ')')
    console.log("[initRealtimeSubscriptions] Фильтр подписки: user_id=eq." + encodedLogistId)
  ...
  // Функция проверки минусового баланса
  const checkNegativeBalance = useCallback(async () => {
    // Загружаем актуальный баланс перед проверкой
    const balanceResp = await loadUserBalance()
    if (!balanceResp?.success) {
      // Если не удалось загрузить баланс, пропускаем проверку
      return false
    }
    
    // Получаем баланс из ответа
    const balanceValue = balanceResp.balance?.available ?? 0
    const currentBalance = Number(balanceValue)
    
    if (currentBalance < 0) {
      await showAlert(
        "Минусовой баланс", 
        "У тебя минусовой баланс что бы дальше пользоваться платформой пополните баланс!!!"
      )
      return true // Баланс отрицательный
    }
    return false // Баланс в порядке
  }, [loadUserBalance, showAlert])
  ...
  const loadNotifications = useCallback(async (silent = false) => {
    ...
    try {
      // Передаем не закодированный userId - backend сам закодирует его для поиска в БД
      console.log("[loadNotifications] userId:", userId, "role:", LOGIST_ROLE)
      const resp = await callApi({
      ...
      if (resp?.success && resp.notifications) {
        const filteredNotifications = (resp.notifications || []).filter(isLogistNotification)
        const now = new Date()
        
        setNotifications(prev => {
          const prevIds = new Set(prev.map(n => String(n.id)))
          
          const newUnreadNotifications = filteredNotifications.filter(n => {
          ...
          if (newUnreadNotifications.length > 0 && !silent) {
            if (!areNotificationsEnabled()) {
              ...
              setCurrentToasts(prevToasts => {
                const existingIds = new Set(prevToasts.map(t => String(t.id)))
                ...
                if (toAdd.length > 0) {
                  ...
                  const newToasts = [...prevToasts, ...toAdd]
                  const result = newToasts.slice(-MAX_TOASTS)
  ...
  const deleteNotification = useCallback(async (notificationId) => {
    ...
    try {
      ...
      if (resp?.success) {
        ...
        // Также удаляем из toast'ов если есть
        setCurrentToasts(prev => prev.filter(t => t.id !== notificationId))
      } else {
        console.error("[deleteNotification] Ошибка:", resp?.error)
      }
    } catch (error) {
      console.error("[deleteNotification] Исключение:", error)
    }
  }, [callApi])
